<template>
  <aside
    class="fixed inset-y-0 left-0 z-20 flex flex-col bg-white dark:bg-neutral-900 border-r border-gray-100 dark:border-neutral-800 transition-all duration-200"
    :class="collapsed ? 'w-16' : 'w-60'"
  >
    <!-- Logo -->
    <div class="flex items-center gap-3 px-4 h-14 border-b border-gray-100 dark:border-neutral-800 flex-shrink-0">
    ...
    <nav class="flex-1 overflow-y-auto py-3 px-2">
      <template v-for="section in menu" :key="section.key">
        ...
        <p
          ...
          class="px-2 pt-4 pb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-neutral-500"
        >
          {{ section.label }}
        </p>
        <div v-else-if="collapsed && section.isTitle" class="mt-3 border-t border-gray-100 dark:border-neutral-800" />

        <!-- Nav item with children -->
        <template v-if="!section.isTitle && section.children">
          <button
            class="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm text-gray-600 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-800 hover:text-gray-900 dark:hover:text-neutral-100 transition-colors"
            :class="{ 'bg-gray-50 dark:bg-neutral-800 text-gray-900 dark:text-neutral-100': isGroupActive(section) }"
          ...
          >
            ...
            <Icon
              ...
              class="text-xs text-gray-400 dark:text-neutral-500 transition-transform"
              :class="{ 'rotate-90': openGroups.has(section.key) }"
            />
          </button>
          <div v-if="!collapsed && openGroups.has(section.key)" class="ml-4 mt-0.5 mb-1 border-l border-gray-100 dark:border-neutral-800 pl-3 space-y-0.5">
            <RouterLink
              v-for="child in section.children"
              :key="child.key"
              :to="child.url || '/'"
              class="flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-colors"
              :class="isActive(child.url) ? 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 font-medium' : 'text-gray-600 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-800 hover:text-gray-900 dark:hover:text-neutral-100'"
            >
        ...
        <RouterLink
          ...
          :class="isActive(section.url) ? 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 font-medium' : 'text-gray-600 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-800 hover:text-gray-900 dark:hover:text-neutral-100'"
        >
          <Icon :icon="section.icon || 'lucide:circle'" class="text-base flex-shrink-0" />
    ...
    <!-- Bottom: user logout -->
    <div class="flex-shrink-0 border-t border-gray-100 dark:border-neutral-800 p-2">
      <RouterLink
        to="/auth/logout"
        class="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm text-gray-500 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-800 hover:text-gray-900 dark:hover:text-neutral-100 transition-colors"
      >
