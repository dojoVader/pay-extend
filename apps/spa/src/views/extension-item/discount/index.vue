<template>
  ...
  <div class="flex items-center justify-between mb-4">
    <h3 class="text-sm font-semibold text-gray-900">Discounts</h3>
  ...
  <div v-else class="card p-6 text-center">
    <p class="text-sm text-gray-400">No discounts configured for this extension.</p>
  </div>

  <!-- Detail modal -->
  <Dialog v-model:visible="detailVisible" modal header="Discount Detail" :style="{ width: '480px' }">
    <div v-if="selectedDiscount" class="space-y-3 text-sm">
      ...
      <div class="flex justify-between">
        ...
        <span>{{ selectedDiscount.type }}</span>
      </div>
      <div class="flex justify-between">
      ...
      <div class="flex justify-between">
        ...
        <span>{{ selectedDiscount.duration }}</span>
      </div>
      <div class="flex justify-between">
  ...
  <!-- Add / Update drawer -->
  <Drawer v-model:visible="drawerOpen" :header="editingId ? 'Update Discount' : 'Add Discount'" position="right" :style="{ width: '400px', fontSize: '14px' }">
    <form class="space-y-4" @submit.prevent="handleSubmit">
      ...
      <div>
        <input type="hidden" :value="props.extensionId" name="extensionId" />
      </div>
...
async function fetchDiscounts() {
  try {
    const url = props.extensionId
      ? `${PAYEXTEND_BASE_URL}polar/discount/mappings/${props.extensionId}`
      : `${PAYEXTEND_BASE_URL}polar/discounts`
...
function openUpdate(discount: Discount) {
  ...
  form.extensionId = props.extensionId ?? ''
...
async function handleSubmit() {
  ...
  try {
    const payload: Record<string, unknown> = {
      name: form.name,
      type: form.type,
      amount: Number(form.amount) * 100, // Convert dollars to cents if fixed
...
:root{
  ...
  --p-drawer-title-font-size: 16px !important;
}
</style>