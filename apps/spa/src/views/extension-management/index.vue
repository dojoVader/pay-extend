    ...
    <div class="card overflow-hidden">
      ...
      <div class="flex flex-wrap items-center gap-3 p-4 border-b border-gray-100">
        <div class="relative">
          ...
          <input type="text" v-model="search" placeholder="Search extensions..." class="form-input pl-9 py-1.5 text-sm w-56" />
        </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="item in filtered" :key="item.id">
              ...
              <td>
                <span class="w-8 h-4 rounded-full inline-flex items-center px-1 text-xs" :class="item.active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'">
                  {{ item.active ? 'On' : 'Off' }}
                </span>
              </td>
      ...
      <div class="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
        <p class="text-xs text-gray-400">Showing <b class="text-gray-600">{{ filtered.length }}</b> of {{ items.length }} extensions</p>
      </div>
    </div>
          ...
          <div class="flex-1 overflow-y-auto p-5">
            <form class="space-y-4" @submit.prevent="handleSubmit">
              ...
              <div class="flex items-center gap-2">
                <input type="checkbox" v-model="form.active" id="active-toggle" class="w-4 h-4 rounded text-indigo-600" />
